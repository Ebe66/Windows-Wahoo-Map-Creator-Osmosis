...
def main(args):
    ...
    print("Bounding box:")
    print(" - minLat: {:f} minLon: {:f}".format(r.readDegrees(),r.readDegrees()))
    print(" - maxLat: {:f} maxLon: {:f}".format(r.readDegrees(),r.readDegrees()))
    ...
    if (flags&0x80) != 0:
        print(" - Debug file")
    if (flags&0x40) != 0:
        print(" - Map start position: lat: {:f} lon: {:f}", r.readDegrees(), r.readDegrees())
    if (flags&0x20) != 0:
    ...
    if args.zoom_intervals:
        for zi in zoomIntervals:
            print(" - Base: {} Min: {} Max: {} Pos: {} Size: {}".format(zi[0], zi[1], zi[2], zi[3], zi[4]))

    r.close()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('-p', "--pois", help="show poi tags", action="store_true")
    parser.add_argument('-w', "--ways", help="show way tags", action="store_true")
    parser.add_argument('-z', "--zoom-intervals", help="show zoom intervals", action="store_true")
